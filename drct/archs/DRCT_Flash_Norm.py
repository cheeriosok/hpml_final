...
class WindowAttention(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # Generate relative position bias table
        ...
        relative_coords_table = torch.stack(torch.meshgrid([relative_coords_h, relative_coords_w])).permute(1, 2, 0).contiguous()

        # Normalize the relative coordinates
        relative_coords_table[:, :, 0] /= (window_size[0] - 1)
        relative_coords_table[:, :, 1] /= (window_size[1] - 1)
        relative_coords_table *= 8  # Normalize to range -8, 8
        relative_coords_table = torch.sign(relative_coords_table) * torch.log2(torch.abs(relative_coords_table) + 1) / torch.log2(torch.tensor(8.0))

        self.register_buffer("relative_coords_table", relative_coords_table.view(-1, 2))
        ...
        # Pair-wise relative position index for each token in the window
    ...
    def forward(self, x, mask=None):
        ...
        q = q.reshape(-1, q.size(-2), q.size(-1)).to(torch.float16)  # Convert to fp16
        k = k.reshape(-1, k.size(-2), k.size(-1)).to(torch.float16)  # Convert to fp16
        v = v.reshape(-1, v.size(-2), v.size(-1)).to(torch.float16)  # Convert to fp16
        ...
        attn = attn.view(B_, self.num_heads, N, -1).transpose(1, 2).reshape(B_, N, -1)
    ...
    def flops(self, N):
        # Calculate FLOPs for a single window with token length of N
        flops = 0
        # qkv = self.qkv(x)
